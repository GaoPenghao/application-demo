...
struct sqp_settings_t {
  ...
  Scalar eps_prim =
      1e-4; /**< primal step termination threshold, eps_prim > 0 */
...
enum class Status { SOLVED, MAX_ITER_EXCEEDED, INVALID_SETTINGS };
...
class NonLinearProblem {
 public:
  ...
  // NonLinearProblem(int num_var, int num_constr)
  //     : num_var_(num_var), num_constr_(num_constr) {}

  virtual void init_nlp() = 0;

  virtual void get_starting_point(Vector& x0, Vector& y0) = 0;

  virtual void objective(const Vector& x, Scalar& obj) = 0;
  virtual void grad_f(const Vector& x, Vector& grad) = 0;
  virtual void objective_linearized(const Vector& x, Vector& grad,
                                    Scalar& obj) = 0;
  virtual void constraint(const Vector& x, Vector& c, Vector& l, Vector& u) = 0;
  virtual void jac_g(const Vector& x, Matrix& Jc) = 0;
  virtual void constraint_linearized(const Vector& x, Matrix& Jc, Vector& c,
                                     Vector& l, Vector& u) = 0;
...
class SQP {
 public:
  ...
  // Constants
  static constexpr Scalar DIV_BY_ZERO_REGUL =
  ...
  bool run_solve_qp(const Matrix& P, const Vector& q, const Matrix& A,
  ...
  Scalar constraint_norm(const Vector& constr, const Vector& l,
