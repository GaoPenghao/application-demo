...
struct QPSolverSettings {
  ...
  Scalar alpha =
      1.0;               /**< ADMM overrelaxation parameter, 0 < alpha < 2,
  ...
  int check_termination = 25; /**< Check termination after every Nth iteration,
                                 0 (disabled) or 0 < check_termination */
  ...
  int adaptive_rho_interval =
      25; /**< change rho every Nth iteration, 0 < adaptive_rho_interval,
...
template <typename Scalar>
struct QPSolverInfo {
  QPSolverStatus status = QPSolverStatus::UNINITIALIZED; /**< Solver status */
  ...
  void print() const {
    ...
    printf("  status ");
    switch (status) {
      case QPSolverStatus::SOLVED:
        printf("SOLVED\n");
        break;
      case QPSolverStatus::MAX_ITER_EXCEEDED:
        printf("MAX_ITER_EXCEEDED\n");
        break;
      case QPSolverStatus::UNSOLVED:
        printf("UNSOLVED\n");
        break;
      case QPSolverStatus::NUMERICAL_ISSUES:
...
class QPSolver {
 public:
  ...
  static constexpr Scalar MY_RHO_MIN = 1e-6;
  static constexpr Scalar MY_RHO_MAX = 1e+6;
  static constexpr Scalar MY_RHO_TOL = 1e-4;
  static constexpr Scalar MY_RHO_EQ_FACTOR = 1e+3;
  static constexpr Scalar MY_LOOSE_BOUNDS_THRESH = 1e+16;
  static constexpr Scalar MY_DIV_BY_ZERO_REGUL =
