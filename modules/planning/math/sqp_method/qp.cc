...
void QPSolver<T>::setup(const QP &qp) {
  ...
  if (compute_KKT()) {
    info_.status = QPSolverStatus::UNSOLVED;
  } else {
    info_.status = QPSolverStatus::NUMERICAL_ISSUES;
  }
}
...
void QPSolver<T>::solve(const QP &qp) {
  ...
  if (info_.status == QPSolverStatus::UNINITIALIZED ||
      info_.status == QPSolverStatus::NUMERICAL_ISSUES) {
  ...
  for (iter = 1; iter <= settings_.max_iter; iter++) {
    ...
    if (settings_.check_termination != 0 &&
        iter % settings_.check_termination == 0) {
    ...
    if (check_termination) {
      ...
      if (termination_criteria(qp)) {
        info_.status = QPSolverStatus::SOLVED;
        break;
      }
    }
    ...
    if (settings_.adaptive_rho && iter % settings_.adaptive_rho_interval == 0) {
      ...
      new_rho = fmax(MY_RHO_MIN, fmin(new_rho, MY_RHO_MAX));
  ...
  if (iter > settings_.max_iter) {
    info_.status = QPSolverStatus::MAX_ITER_EXCEEDED;
  }
  info_.iter = iter;
...
#else
  kkt_mat.topLeftCorner(n, n) =
      *qp.P + settings_.sigma * Matrix::Identity(n, n);
...
#ifdef QP_SOLVER_USE_SPARSE
  // Note: optimize by writing kkt_mat.valuePtr(). needs to be compressed and
  // Eigen::Lower.
  for (int i = 0; i < m; i++) {
...
#ifdef QP_SOLVER_USE_SPARSE
template <typename T>
void QPSolver<T>::sparse_insert_at(Matrix &dst, int row, int col,
                                   const Matrix &src) const {
...
template <typename T>
void QPSolver<T>::constr_type_init(const Vector &l, const Vector &u,
                                   Eigen::VectorXi &constr_type) {
  for (int i = 0; i < l.rows(); i++) {
    if (l[i] < -MY_LOOSE_BOUNDS_THRESH && u[i] > MY_LOOSE_BOUNDS_THRESH) {
      constr_type[i] = static_cast<int>(ConstraintType::LOOSE_BOUNDS);
    } else if (u[i] - l[i] < MY_RHO_TOL) {
      constr_type[i] = static_cast<int>(ConstraintType::EQUALITY_CONSTRAINT);
    } else {
      constr_type[i] = static_cast<int>(ConstraintType::INEQUALITY_CONSTRAINT);
    }
  }
}
...
void QPSolver<T>::rho_vec_update(Scalar rho0) {
  for (int i = 0; i < rho_vec.rows(); i++) {
    switch (constr_type[i]) {
      case static_cast<int>(ConstraintType::LOOSE_BOUNDS):
        rho_vec[i] = MY_RHO_MIN;
        break;
      case static_cast<int>(ConstraintType::EQUALITY_CONSTRAINT):
        rho_vec[i] = MY_RHO_EQ_FACTOR * rho0;
        break;
      case static_cast<int>(
          ConstraintType::INEQUALITY_CONSTRAINT): /* fall through */
      default:
        rho_vec[i] = rho0;
    };
  }
...
template <typename T>
typename QPSolver<T>::Scalar QPSolver<T>::rho_estimate(const Scalar rho0,
                                                       const QP &qp) const {
  Scalar rp_norm, rd_norm;
  rp_norm = info_.res_prim / (max_Ax_z_norm_ + MY_DIV_BY_ZERO_REGUL);
  rd_norm = info_.res_dual / (max_Px_ATy_q_norm_ + MY_DIV_BY_ZERO_REGUL);

  Scalar rho_new = rho0 * sqrt(rp_norm / (rd_norm + MY_DIV_BY_ZERO_REGUL));
  return rho_new;
}
