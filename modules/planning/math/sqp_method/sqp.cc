...
void SQP<T>::solve(Problem& prob) {
  const int nx = prob.num_var_;
  const int nc = prob.num_constr_;
...
void SQP<T>::run_solve(Problem& prob) {
  ...
  for (iter = 1; iter <= settings_.max_iter; iter++) {
    ...
    if (termination_criteria(x_, prob)) {
      info_.status = Status::SOLVED;
      break;
    }
  }
  if (iter > settings_.max_iter) {
    info_.status = Status::MAX_ITER_EXCEEDED;
  }
}
...
bool SQP<T>::termination_criteria(const Vector& x, Problem& prob) {
  if (primal_step_norm_ <= settings_.eps_prim &&
      dual_step_norm_ <= settings_.eps_dual &&
...
template <typename T>
bool SQP<T>::run_solve_qp(const Matrix& P, const Vector& q, const Matrix& A,
                          const Vector& l, const Vector& u, Vector& prim,
                          Vector& dual) {
  QuadraticProblem<Scalar> qp_;
  ...
  if (qp_solver_.info().status == QPSolverStatus::NUMERICAL_ISSUES) {
...
void SQP<T>::second_order_correction(Problem& prob, Vector& p, Vector& lambda) {
  ...
  // mu = (grad_obj_.dot(p) + 0.5 * p.dot(Hess_ * p)) / ((1 - settings_.rho) *
  // constr_l1); phi_l1 = obj_ + mu * constr_l1;
...
typename SQP<T>::Scalar SQP<T>::line_search(Problem& prob, const Vector& p) {
  ...
  const Scalar tau =
      settings_.tau;  // line search step decrease, 0 < tau < settings.tau

  Scalar constr_l1 = constraint_norm(constr_, l_, u_);

  // get mu from merit function model using hessian of Lagrangian instead
  mu = (grad_obj_.dot(p) + 0.5 * p.dot(Hess_ * p)) /
       ((1 - settings_.rho) * constr_l1);
...
template <typename T>
typename SQP<T>::Scalar SQP<T>::constraint_norm(const Vector& constr,
                                                const Vector& l,
...
template <typename T>
typename SQP<T>::Scalar SQP<T>::constraint_norm(const Vector& x,
                                                Problem& prob) {
...
template <typename T>
typename SQP<T>::Scalar SQP<T>::max_constraint_violation(const Vector& x,
                                                         Problem& prob) {
  // Note: uses members constr_, l_ and u_ as temporary
